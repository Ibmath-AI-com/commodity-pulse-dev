...
export async function POST(req: NextRequest) {
  try {
    ...
    const futureDate = String(body?.futureDate ?? "").trim(); // e.g. "2026-01-20" (prefer ISO)
    ...
    // Mirror /api/predict behavior
    const baseWebhookUrl = requiredEnv("N8N_WEBHOOK_GENERATING_REPORT_URL");
